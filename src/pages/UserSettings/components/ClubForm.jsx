...
import { clubSchema } from "../../../schema/userSchema";
import { yupResolver } from "@hookform/resolvers/yup";
import { editUser } from "../../../api/users";

function ClubForm({ user, setToggleSettings, setUser }) {
  const defaultValues = {
  ...
  return (
    <>
      <form className="my-4 flex flex-row justify-between items-center" onSubmit={handleSubmit(submit)}>
        <label className="font-semibold" htmlFor="club">
          Club
        </label>
        <div className="flex flex-row">
          <input type="text" name="club" {...register("club")} className="mr-2 input input-large" />
        </div>
